...
export class PlaneService {
    ...
    async findAll(query: PlaneTicketQueryDto): Promise<{
        ...
        const filter: any = { ticket_type: TicketTypes.PLANE, is_deleted: { $ne: true } };
        ...
        if (destination_location) {
            filter.destination_location = { $regex: destination_location, $options: 'i' };
        }
    ...
    async findById(id: string): Promise<ITicket> {
        ...
        const ticket = await this.ticketModel
            .findOne({ _id: id, ticket_type: TicketTypes.PLANE, is_deleted: { $ne: true } })
            .populate('employee', 'email')
            .populate('agency')
    ...
    async update(id: string, updatePlaneTicketDto: UpdatePlaneTicketDto): Promise<ITicket> {
        if (!Types.ObjectId.isValid(id)) {
            throw new BadRequestException('Invalid ticket ID');
        }
