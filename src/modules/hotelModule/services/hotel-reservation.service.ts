...
import { IHotelReservation, ReservationStatus } from 'src/shared/types/hotel.types';
import {
...
export class HotelReservationService {
    constructor(
        @InjectModel('HotelReservation') private reservationModel: Model<IHotelReservation>,
    ) { }

    async create(createReservationDto: CreateHotelReservationDto): Promise<IHotelReservation> {
        ...
        return await this.findById(savedReservation._id.toString(), createReservationDto.agency);
    }
    ...
    async findByBookingId(bookingId: string, agencyId: string): Promise<IHotelReservation> {
        const reservation = await this.reservationModel
            .findOne({
    ...
    async update(id: string, agencyId: string, updateReservationDto: UpdateHotelReservationDto): Promise<IHotelReservation> {
        if (!Types.ObjectId.isValid(id)) {
            throw new BadRequestException('Invalid reservation ID');
        }
        ...
        if (updateReservationDto.hotel_partner) {
            updateData.hotel_partner = new Types.ObjectId(updateReservationDto.hotel_partner);
        }
    ...
    async addLog(id: string, agencyId: string, addLogDto: AddReservationLogDto): Promise<IHotelReservation> {
        if (!Types.ObjectId.isValid(id)) {
            throw new BadRequestException('Invalid reservation ID');
        }
    ...
    async updateStatus(id: string, agencyId: string, status: ReservationStatus): Promise<IHotelReservation> {
        if (!Types.ObjectId.isValid(id)) {
            throw new BadRequestException('Invalid reservation ID');
        }
