...
export class PartnerHotelService {
    constructor(
        @InjectModel('PartnerHotel') private partnerHotelModel: Model<IPartnerHotel>,
    ) { }

    async create(createPartnerHotelDto: CreatePartnerHotelDto): Promise<IPartnerHotel> {
    ...
    async update(id: string, agencyId: string, updatePartnerHotelDto: UpdatePartnerHotelDto): Promise<IPartnerHotel> {
        if (!Types.ObjectId.isValid(id)) {
            throw new BadRequestException('Invalid partner hotel ID');
        }
