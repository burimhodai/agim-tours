...
export class LoyalCustomerService {
    constructor(
        @InjectModel('LoyalCustomer') private readonly loyalCustomerModel: Model<any>,
    ...
    async update(id: string, data: UpdateLoyalCustomerDto, agency: any) {
        const agencyId = await this.getAgencyId(agency);
        return this.loyalCustomerModel.findOneAndUpdate(
    ...
    async addPurchase(customerId: string, purchaseData: AddPurchaseDto, agency: any) {
        const agencyId = await this.getAgencyId(agency);
        const ticketRefModel = purchaseData.ticket_type === 'bus' || purchaseData.ticket_type === 'plane'
            ? 'Ticket'
            ...
            { _id: new Types.ObjectId(customerId), agency: agencyId },
            {
                $push: {
                    purchase_history: {
                        ...
                        ticket_id: purchaseData.ticket_id ? new Types.ObjectId(purchaseData.ticket_id) : undefined,
