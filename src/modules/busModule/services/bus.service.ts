...
export class BusService {
    ...
    async findAll(query: BusTicketQueryDto): Promise<{
        ...
        const filter: any = { ticket_type: TicketTypes.BUS, is_deleted: { $ne: true } };
    ...
    async findById(id: string): Promise<ITicket> {
        ...
        const ticket = await this.ticketModel
            .findOne({ _id: id, ticket_type: TicketTypes.BUS, is_deleted: { $ne: true } })
            .populate('employee', 'email')
            .populate('agency')
    ...
    async update(id: string, updateBusTicketDto: UpdateBusTicketDto): Promise<ITicket> {
        if (!Types.ObjectId.isValid(id)) {
            throw new BadRequestException('Invalid ticket ID');
        }
