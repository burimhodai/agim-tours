...
export class SeedDataService {
  ...
  async seedReportData() {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const routeList =
        ticketType === TicketTypes.BUS ? routes.bus : routes.plane;
      ...
      const price =
        basePrice + Math.random() * (ticketType === TicketTypes.BUS ? 70 : 300);
      ...
      arrivalDate.setHours(
        departureDate.getHours() + (ticketType === TicketTypes.BUS ? 20 : 3),
      );
    ...
    console.log(
      `   - Created ${standaloneTransactions.length} standalone transactions for today`,
    );
